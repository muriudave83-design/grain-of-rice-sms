...
import { deleteOwnedAssignment, SCORE_DELETE_CONFIRMATION } from "./assignmentDeletion.service";
  ...
  const client = {
    ...
    $transaction: async (callback: any, config: any) => {
      ...
      const tx = {
        ...
        grade: { findFirst: async (args: any) => { calls.push(`grade.find:${JSON.stringify(args.where)}`); return options.grade ?? null; } },
...
test("teacher deletes own zero-score assignment transactionally without destructive phrase", async () => {
  ...
  assert.deepEqual(await deleteOwnedAssignment(client as any, 30, 10), { status: "DELETED", scoreCount: 0, title: "Assignment A" });
...
test("published Grade blocks deletion regardless of Assignment lock state", async () => {
  const client = clientFor({ assignment: assignment({ isLocked: true, _count: { scores: 15 } }), grade: { id: 90 } });
  assert.deepEqual(await deleteOwnedAssignment(client as any, 30, 10, SCORE_DELETE_CONFIRMATION), { status: "PUBLISHED", scoreCount: 15 });
  assert.doesNotMatch(client.calls.join("\n"), /deleteMany|assignment\.delete/);
  const gradeQuery = client.calls.find((call) => call.startsWith("grade.find:")) || "";
